...
namespace prob
{
   ...
   struct MaliciousPathIteration
   {
      using Graph = MaliciousPath::Graph;

      MaliciousPathIteration(Graph const* graph)
         : m_graph(graph)
         , m_processed(graph->size(), false)
         , m_maxCosts(graph->size(), -1)
         , m_minLinks(graph->size(), -1)
      {}

      MaliciousPathIteration next(std::vector<int> const& nextJump)
      {
         MaliciousPathIteration next(m_graph);
         next.m_processed[m_graph->size() - 1] = true;
         next.m_maxCosts[m_graph->size() - 1] = 0;

         for (size_t v = 0; v < m_graph->size(); ++v)
            computeNextAt(next, v, nextJump);
         return next;
      }

      long computeNextAt(MaliciousPathIteration& next, int current, std::vector<int> const& nextJump)
      {
         if (next.m_processed[current])
            return next.m_maxCosts[current];

         next.m_processed[current] = true;
         for (auto const& e : m_graph->at(current))
         {
            long newCost = e.m_cost;

            //Not using any token in case we are on the shortest path - THIS CONDITION IS WRONG WITH MULTIPLE EDGES
            if (e.m_to == nextJump.at(current))
            {
               newCost += computeNextAt(next, e.m_to, nextJump);
            }
            //Using a token otherwise
            else
            {
               newCost += m_maxCosts[e.m_to];
            }

            if (newCost > next.m_maxCosts[current])
            {
               next.m_maxCosts[current] = newCost;
               next.m_minLinks[current] = m_minLinks[e.m_to] + 1;
            }
         }
         return next.m_maxCosts[current];
      }
      ...
      std::vector<long> m_maxCosts;
      std::vector<long> m_minLinks;
   };

   long MaliciousPath::minPath(int N, int K, std::vector<int> const& from, std::vector<int> const& to, std::vector<int> const& cost)
   {
      Graph graph(N);
      Graph reversedGraph(N);
      for (size_t v = 0; v < from.size(); ++v)
      {
         graph[from[v]].emplace_back(to[v], cost[v]);
         reversedGraph[to[v]].emplace_back(from[v], cost[v]);
      }

      std::vector<int> nextJump(N, -1);
      MaliciousPathIteration current(&graph);
      shortestPathsFrom(reversedGraph, N - 1, current.m_maxCosts, current.m_minLinks, nextJump);
      for (int token = 1; token <= K; ++token)
         current = current.next(nextJump);

      if (current.m_minLinks.at(0) > 1000000000)
         return -1;
      return current.m_maxCosts.at(0);
   }

   void MaliciousPath::shortestPathsFrom(Graph const& graph, int source, std::vector<long>& shortestLen, std::vector<long>& nbLinks, std::vector<int>& prevOnPath)
   {
      using CostTo = WeightedEdge;
      auto greater = [](CostTo const& lhs, CostTo const& rhs) { return lhs.m_cost > rhs.m_cost; };
      std::priority_queue<CostTo, std::vector<CostTo>, decltype(greater)> queue(greater);

      std::vector<bool> processed(graph.size(), false);
      std::fill(begin(shortestLen), end(shortestLen), std::numeric_limits<int>::max());
      std::fill(begin(nbLinks), end(nbLinks), std::numeric_limits<int>::max());
      ...
      while (!queue.empty())
      {
         CostTo closest = queue.top();
         queue.pop();
         int curr = closest.m_to;
         processed[curr] = true;

         for (auto const& e : graph[curr])
         {
            auto next = e.m_to;
            if (processed[next])
               continue;

            int cost = e.m_cost + closest.m_cost;
            int linkCount = 1 + nbLinks.at(curr);
            if (cost <= shortestLen.at(next))
            {
               shortestLen.at(next) = cost;
               if (linkCount < nbLinks.at(next))
               {
                  nbLinks.at(next) = linkCount;
                  prevOnPath.at(next) = curr;
               }
               queue.emplace(next, cost);
            }
         }
      }
   }
